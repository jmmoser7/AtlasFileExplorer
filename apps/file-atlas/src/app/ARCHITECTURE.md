...
| Canvas | `mod.rs` (`canvas`) + `atlas-shell::folder_map` | Infinite map, selection, thumbnails. The tree paint path (leaders, collapse grips, group-preview mosaics, file cards, `FolderCam`) and the packed image sheet live in `atlas-shell::folder_map` so the Slate File Atlas portal cannot drift. Layout of the packed sheet is `atlas_core::pack_sheet` (sort + square pack, no gaps). **Remaining Art. XII debt:** scan/watcher session and canvas input still live in this `mod.rs`; the portal has a simplified copy in `board_atlas.rs` (DV-19) — extract a shared session before the next perf fix. Draws the scanned tree only — the parent-folder chain above the mapped root was removed as clutter, so `map_bounds` is the tree's own bounds (or the sheet bounds in packed view). Collapsed folders above the **stack threshold** paint as **group previews** (internal `portal_threshold` / `shows_portal` — not Slate portals). Filters' *Zoom to matches* (`auto_zoom_after_filter`, on by default) flies the camera to the surviving files, edge-triggered on the framed bounds so scan batches do not yank it. Packed view applies the same `file_match` set, the same recorded collapse / stack threshold (one cover tile per large collapsed folder), a sort, and a field-aspect pack. |
