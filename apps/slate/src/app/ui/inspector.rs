...
fn body(app: &mut SlateApp, ui: &mut egui::Ui, theme: SidebarTheme) {
    ...
    let tool_group = Id::new(("slate_selection_tools", ids[0].0));

    // ----- common: opacity + actions -----
    sidebar_collapsible_region(ui, tool_group, Id::new("opacity"), "Opacity", theme, |ui| {
        opacity_controls(app, ui, theme, &ids, &primary);
    });
    ...
    match &primary.kind {
        NodeKind::Shape(s) => {
            ...
            sidebar_collapsible_region(
                ...
                stroke_label,
                theme,
                |ui| stroke_controls(app, ui, theme, &ids, &primary),
            );
        ...
        NodeKind::Text(_) => {
            sidebar_collapsible_region(ui, tool_group, Id::new("text"), "Text", theme, |ui| {
                text_controls(app, ui, theme, &ids, &primary)
            });
        }
        NodeKind::Frame(_) => {
            sidebar_collapsible_region(ui, tool_group, Id::new("frame"), "Frame", theme, |ui| {
...
fn portal_controls(
    ...
    match p.kind {
        PortalKind::Agent => agent_portal_controls(app, ui, theme, ids, primary),
        PortalKind::Web => web_portal_controls(app, ui, theme, ids, primary),
        PortalKind::FileAtlas => atlas_portal_controls(app, ui, theme, ids, primary),
    }
}
