...
| D09 | Preview & readouts | No second frame — each selected shape keeps its own silhouette. Icons sit `align.frame_outset` (20 px) outside the union box on the left; the bottom cluster also clears the width stringer (`STRINGER_GAP × zoom`). Hover: icon invert + action name outside the cluster. No ghost outlines, no axis guide (they flicker). No numeric readout. | stated | 100 |
...
| D13 | Selected presentation | Selection chrome is a per-shape silhouette (no corner or midspan squares, no painted union box). Resize and rotate remain hover-hit on the union box. Two clusters only: bottom = left / center-h / right / distribute-h; left = top / center-v / bottom / distribute-v. Icons sit outside the box (and past the width stringer on the bottom) so they miss the edge-handle hit. | stated | 100 |
...
| `align.frame_outset` | Left-cluster offset outside the group box | 20.0 |
| `align.bottom_outset` | Bottom-cluster offset: `max(frame_outset, STRINGER_GAP×zoom + icon/2 + 10×zoom)` | dynamic |
...
- **GP5 (widget hit misses the box):** 2+ selected · screen point on the inner group-box bottom edge → `align_action_at` is `None`; point on the bottom-cluster left icon (at `bottom_outset_px(zoom)`) → `Align(Left)`. The old top-cluster position is `None`.
