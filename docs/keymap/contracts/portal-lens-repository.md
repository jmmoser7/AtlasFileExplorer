...
| D13 | Selected presentation | Windows-style hover resize on the frame (P1.node.transform) — no prior selection. Contents expose no grips and are not selectable — a generated portal owns no mutations (Art. V.3). Resize re-lays-out, it does not scale: frame size is a layout input, so a wider frame shows more history rather than a stretched picture. Portals stay axis-aligned: no rotate chrome. | stated | 100 |
...
| D17 | Hit-testing & pick | The frame picks on its rect (Frame-node semantics), including marquee. Contents become interactive only when the portal is focused **and** its apparent scale passes `portal.repo.interact_zoom`. Double-click (or Enter) focuses the contents. **P1.portal.maximize:** the four-corner maximize square in the upper-right fills the window. In focused/interactive mode, commit dots and ribbons hit-test with `pick.slop` (4 px). Clicking outside or Esc (after leaving maximize) drops contents focus. | stated | 100 |
...
| D33 | Portal chrome | **No identity tab** (web-only). Maximize is the four-corner square in the upper-right. Right-click: Maximize (plus existing repo actions). | stated | 100 |
| D34 | Portal maximize | **P1.portal.maximize.** Fills the window at the screen aspect; Esc restores. Contents fit the screen body. | stated | 100 |
| D35 | Portal-local UI | **P1.portal.local-ui.** Query knobs (D20) and repo actions live on this portal's inspector. None appear in Document Settings. | stated | 100 |
