...
| D23 | Level of detail | Buckets key on the portal's on-screen height in physical pixels, so a hundred tiled pages zoomed out cost nothing. Below `portal.web.lod_strip_px` (96): chrome strip only, no texture bound. From 96 to `portal.web.live_min_px` (160), or off-screen at any size: the cached poster. At or above 160 and on screen: **eligible** for the live pool, admitted by the D29 priority; an eligible portal that misses admission paints its poster and reports `Budgeted`. Double-clicking a portal below the live threshold zooms the camera to fit it first, then takes input focus — and a focused portal ignores the size gate entirely, so the human is never told "too small" after asking to run the page. | pattern | 75 |
...
| D31 | View-state ownership | Journaled authored intent: frame rect / rotation / opacity / fill, title, locator, entry, viewport width and zoom mode, `interactive_allowed`, `poster_capture` mode, export mode. Derived per-peer: poster texture and its capture time, live/pool membership, input focus, scroll offset, page navigation history, load errors, origin consent grants, WebView2 runtime availability, **maximize**, **identity-tab fold**. | stated | 100 |
| D32 | Trust, sandbox & consent | Remote origins are `Blocked` until the human permits them; grants key on (workbook, origin), live in local app state, and are **never journaled or written into the `.slate`**, so a workbook you receive cannot arrive pre-trusting a host. Placing a default web portal is the human permitting the start locator's origin for that workbook, matching paste/drop behavior. Local sources under the workbook root need no prompt. The webview runs in a per-workbook user-data folder — no cookies, storage, or logins shared with the user's real browser. Denied outright: downloads, popups, notifications / geolocation / camera / mic / clipboard-read, and any host object or `postMessage` bridge (D15). Exports sandbox what they embed: packaged local content gets `sandbox="allow-scripts allow-same-origin"`, because a dashboard must fetch its own sibling data files; remote iframes get `allow-scripts` only. | stated | 100 |
| D33 | Portal chrome | **P1.portal.chrome.** One Slate identity tab shows the locator (or the page being visited). The strip is 40% the height of the Slate / File Atlas top bar. Text is clipped to the tab content bounds; truncation is allowed, oversail is not. Maximize is the only chrome button on that strip (glyph highlight, no fill). Fold via right-click Hide tab or `portal.chrome.toggle`; a folded tab is recovered from the top-interior reveal strip. The four-corner maximize square sits in the tab bar's window-control slot (far right), the same place as the Slate / File Atlas caption maximize. Right-click on the tab or the frame: Copy URL, Paste URL (journaled rebind), Maximize, Hide/Show tab. | stated | 100 |
...
| `portal.web.focused_fps` | Readback / upload cadence for the focused portal | `30` |
