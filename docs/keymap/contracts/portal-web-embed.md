...
| D33 | Portal chrome | **P1.portal.chrome.** A slim, inset Slate identity blister shows the locator with smaller centered type. The top bar overlays the full-bleed page; it never reserves browser viewport space. It retracts after 1.2 seconds without pointer, wheel, or keyboard activity and hides when the pointer leaves. Page interaction or hovering the top bar area reveals it again. Idle portals paint no tab, reveal hint, or maximize glyph. Revealing/folding chrome does not resize, reload, or move the page. Maximize remains at the far right when revealed; Escape restores a maximized portal even with hidden chrome. Explicit Hide/Show tab remains available from the portal context menu and portal.chrome.toggle; a manually folded tab uses its top-interior reveal strip. | stated | 100 |
...
| `portal.web.raster_max_px` | Longest-edge cap for a stable live capture | `1920` |
| `portal.web.raster_max_pixels` | Total live capture pixel budget | `2073600` |
