...
use eframe::egui::{
    self, Align, Color32, CornerRadius, Frame, Id, Layout, Margin, RichText, Sense, Ui, Vec2,
};
...
impl SidebarTokens {
    ...
    /// Horizontal inset inside a capsule (minimal).
    pub const INNER_MARGIN_X: f32 = 6.0;
    /// Vertical inset — 40% tighter than the first pass (was 6).
    pub const INNER_MARGIN_Y: f32 = 3.5;
    pub const SECTION_GAP: f32 = 4.0;
    /// ~30% of a collapsed capsule height; pulls the first capsule upward.
...
/// Faint horizontal rule between control-type groups inside a panel body.
...
/// Wrap a same-type control block; optional faint divider before the group.
